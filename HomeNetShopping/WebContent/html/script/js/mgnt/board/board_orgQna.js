...
function onSaveComment(){
	...
	if($("#cmtCn").val().trim().length <= 0) {
		alert(message3);
		$("#cmtCn").focus(); 
		return false;
	}
	if(checkLength($("#cmtCn").val()) > 4000) {
		alert(message14); 
		$("#cmtCn").focus(); 
		return false;
	}
	
	var confrm = confirm(message2);
...
function onDeleteComment(cmtNo){
	var frm = document.form1;
	var confrm = confirm(message1);
	
	if (confrm) {
		frm.cmd.value = "CD";
		frm.cmtNo.value = cmtNo;
		frm.action = url8;
		frm.target = "_self";
		frm.submit();	
	}
