...
function _startLoadXmlTree(sSrc, jsNode) {
	//2007-07-23 다국어 처리를 위해 기존소스 수정.
	
	//다국어작업을 위해 추가.
	var url = extStr(sSrc,0);
	var data = encLang(extStr(sSrc, 1));
	...
	xmlHttp.open("post", url?url:"?", true);	// async
...
function extStr(s, f){
	var p = s.indexOf("?");
	if(p != -1){
		return (f==0)?s.substring(0, p):s.substring(p+1);
	} else {
		return s;
	}
}
...
function encLang(s){
	var sp = 0, cp = 0;
	var key = null,val = null, buf = "";
	if(s.indexOf("&") != -1){
		var toks = s.split("&");
		for(var i=0;i<toks.length;i++){
			if((cp = toks[i].indexOf("=")) != -1){
				key = toks[i].substring(0, cp);
				val = toks[i].substring(cp+1);
				
				if(buf.length > 0){
					buf += "&";
				}
				
				buf += encodeURIComponent(key);
				buf += "="
				buf += encodeURIComponent(val);
			}
		}
	} else {
		buf = s;
