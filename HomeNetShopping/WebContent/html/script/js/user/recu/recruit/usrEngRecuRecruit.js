...
var recuRecruitList = {
	...
	fnTab : function(searchEtrFomCd) {
		...
		frmInit.action = CTX_PATH + "/user/recu/recruit/engRecruitList.do";
		frmInit.submit();
		
	},
	// Tab 검색 - 채용구분
	fnRecuRcmdTab : function(searchRecuRcmdCd) {
		var frmInit = document.formInit;
		
		$("#formInit #searchRecuRcmdCd").val(searchRecuRcmdCd);
		$("#formInit #currentPage").val(1);
	...
	fnListPage : function(pageNo) {
		...
		frm.method = "post";
		frm.target = "_self";
		frm.action = CTX_PATH + "/user/recu/recruit/engRecruitList.do";
	...
	fnView : function(recuInfoNo) {
		...
		frm.method	= "post";
		frm.target	= "_self";
		frm.action	= CTX_PATH + "/user/recu/recruit/engRecruitView.do";
		frm.submit();
	},
	// 신규/수정 페이지로 이동
...
var recuRecruitView = { 
	...
	fnList : function(pageNo) {
		...
		if ( inflowPath != "" ) {
			...
			sUrl = CTX_PATH + "/user/recu/recruit/engRecruitList.do";
		}
	...
	fnRecuApply  : function(submMethCd) {
		...
		if ( submMethCd == '001' ) {
			...
			frm.action = CTX_PATH+"/user/recu/recruit/engRecruitApplyHandlePop.do";
			frm.method = "POST";
			...
			//이력서 선택 팝업
			_RESM_POP = window.open(CTX_PATH+"/user/common/popup/resmEngList.do?callBackFun=fnResmSet", '_RESM_POP' , 'width=800, height=600, resizable=yes, scrollbars=yes, left=200, top=100');
			_RESM_POP.focus();  
		}
	},
	// 폼 초기화 리스트 페이지 이동
