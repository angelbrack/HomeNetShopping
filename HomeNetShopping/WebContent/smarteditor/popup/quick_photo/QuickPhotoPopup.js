	...
	function checkDragAndDropAPI(){
		//(강 - 임시) 모바일 지원을 위해 FILE API가 무조건 드래그 앤 드롭으로 안되게 막아둠
//		try{
//			if( !oNavigator.ie ){
	 ...
	 */
 	function setPhotoToEditor(oFileInfo){
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			...
			//TODO : FileUploadServlet 통해 썸네일이미지를 보여준다.
			var dmUrl = "" ;
			dmUrl = location.protocol ;
			dmUrl += '//'+location.hostname ;
			if(location.port != "" && location.port != "80") {
				dmUrl += ':'+location.port ;
			}
			oFileInfo[0].sFileURL = dmUrl+"/upload?pathkey=EDITOR.IMAGE&getthumb="+oFileInfo[0].sFileName+"&realFileName="+encodeURI(encodeURIComponent(oFileInfo[0].sFileName))+"&curMonth="+oFileInfo[0].curMonth;
