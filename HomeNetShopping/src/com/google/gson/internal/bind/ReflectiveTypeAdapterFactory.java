  ...
  private ReflectiveTypeAdapterFactory.BoundField createBoundField(
    ...
    return new ReflectiveTypeAdapterFactory.BoundField(name, serialize, deserialize) {
      ...
      @Override void read(JsonReader reader, Object value)
        ...
        log.debug("fieldType.getRawType()=["+fieldType.getRawType()+"]");
        log.debug("fieldType.getRawType().getName()=["+fieldType.getRawType().getName()+"]");
        log.debug("name=["+name+"]");
        log.debug("fieldValue_1=["+fieldValue+"]");
        
        if (fieldValue != null || !isPrimitive) {
        	XssHtmlEscape xssHtmlEscape = XssHtmlEscape.getInstance();
        	if ( fieldValue instanceof String ) {
        		fieldValue = xssHtmlEscape.escapeHTML(name, (String)fieldValue);
        	}
        	log.debug("fieldValue_2=["+fieldValue+"]");
